...
{
    static constexpr char digits[] = "000102030405060708090A0B0C0D0E0F"
...
inline std::string format(const char* fmt, ...)
{
    ...
    int n = vsnprintf(&res[0], fmt_buffer_size, fmt, ap);
    ...
    if (static_cast<size_t>(n) >= fmt_buffer_size) {
        fmt_buffer_size = static_cast<size_t>(n) + 1;
        res.resize(fmt_buffer_size);
        va_start(ap, fmt);
        n = vsnprintf(&res[0], fmt_buffer_size, fmt, ap);
        va_end(ap);
    }
